...
class DbObj:
    ...
    def save(self):
        ...
        try:
            ...
            pass  # ignore file-exists or other errors
        f = open(fn, "wb")
        import pickle
        pickle.dump(self, f)
        f.close()
...
class FileCacheRef(MyDict):
    ...
    def FromCacheData(cls, cache_data):
        ref = cls()
        ref.filedepslist = map(lambda fn: (
            fn, getLastChangeUnixTime(fn)), cache_data.filenames)
        ref.macros = {}
...
def check_cache(stateStruct, full_filename):
    ...
    for filecacheref in filecaches:
        ...
        assert filecache is not None, sha1(
            filecacheref) + " not found in " + FileCache.Namespace
        return filecache
...
def State__cached_preprocess(stateStruct, reader, full_filename, filename):
    ...
    if stateStruct._cpre3_atBaseLevel:
        try:
            ...
            print("(Safe to ignore) Error while reading C parser cache for %s : %s" % (
                filename, str(e)))
            # Try to delete old references if possible. Otherwise we might always hit this.
            try:
                FileCacheRefs.Delete(full_filename)
            except:
                pass
...
class StateWrapper:
    WrappedDicts = ("macros", "typedefs", "structs", "unions",
                    "enums", "funcs", "vars", "enumconsts")
    WrappedLists = ("contentlist",)
    LocalAttribs = ("_stateStruct", "_cache_stack", "_additions",
                    "_macroAccessSet", "_macroAddSet", "_filenames", "_cpre3_atBaseLevel")
    ...
    def __getattr__(self, k):
        if k in self.LocalAttribs:
            raise AttributeError  # normally we shouldn't get here but just in case
        if len(self._cache_stack) > 0:
            if k in self.WrappedDicts:
                kwattr = {'d': getattr(self._stateStruct, k),
                          'addList': self._additions[k]}
                if k == "macros":
    ...
    def __setattr__(self, k, v):
        if k in self.LocalAttribs:
            self.__dict__[k] = v
            return
        if k in self.WrappedLists and isinstance(v, StateListWrapper):
            return  # ignore. probably iadd or so.
