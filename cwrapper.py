...
class CStateDictWrapper:
    ...
    def __getitem__(self, k):
        ...
        for f in found:
            ...
            if hasattr(f, "body") and f.body is not None:
                return f
        if found:
...
class CStateWrapper:
    ...
    def __getattr__(self, k):
        ...
        if k == "_errors":
            # fallthrough to CWrapper to collect all errors there
            return getattr(self._cwrapper, k)
        ...
        if len(self._cwrapper.stateStructs) == 0:
            raise AttributeError(
                "CStateWrapper " + str(self) + " doesn't have any state structs set yet")
...
class CWrapper:
    ...
    def register(self, stateStruct, clib):
        ...
        def iterAllAttribs():
            for attrib in stateStruct.macros:
                if stateStruct.macros[attrib].args is not None:
                    continue
                yield attrib
    ...
    def resolveMacro(self, stateStruct, macro):
        ...
        resolvedMacro = macro.getSingleIdentifer(
            self._wrappedStateStruct)  # or just stateStruct?
        if resolvedMacro is not None:
            self.get(str(resolvedMacro))
        return macro
    ...
    def get(self, attrib, resolveMacros=True):
        for stateStruct in self.stateStructs:
            if attrib in stateStruct.macros and stateStruct.macros[attrib].args is None:
                if resolveMacros:
                    return self.resolveMacro(stateStruct, stateStruct.macros[attrib])
                else:
                    return stateStruct.macros[attrib]
