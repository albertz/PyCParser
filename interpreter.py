...
import keyword
PyReservedNames = set(
    dir(sys.modules["__builtin__"]) + keyword.kwlist + ["ctypes", "helpers"])
...
class GlobalScope:
    ...
    def _getVarBodyValueAst(self, decl, decl_type, bodyAst, bodyType):
        ...
        if not isinstance(decl_type, CArrayType) and isPointerType(decl_type) \
            ...
            valueAst = getAstNode_newTypeInstance(
                FuncEnv(self), decl_type, bodyAst, bodyType)
            return valueAst
    ...
    def getVar(self, name):
        ...
        if self.interpreter.debug_print_getVar:
            print("+ getVar %s" % decl)
        assert isinstance(decl, CVarDecl)
        ...
        def getEmpty():
            ...
            v_empty = evalValueAst(
                self, emptyValueAst, "<PyCParser_globalvar_%s_init_empty>" % name)
            self.interpreter._storePtr(ctypes.pointer(v_empty))
            return v_empty
        self.vars[name] = getEmpty()

        bodyValueAst = self._getVarBodyValueAst(
            decl, decl_type, bodyAst, bodyType)
        if bodyValueAst is not None:
            value = evalValueAst(
                self, bodyValueAst, "<PyCParser_globalvar_" + name + "_init_value>")
...
def evalValueAst(funcEnv, valueAst, srccode_name=None):
    if srccode_name is None:
        srccode_name = "<PyCParser_dynamic_eval>"
...
class FuncEnv:
    ...
    def __repr__(self):
        try:
            return "<" + self.__class__.__name__ + " of " + self.get_name() + ">"
        except Exception:
    ...
    def _registerNewVar(self, varName, varDecl):
        ...
        for name in iterIdWithPostfixes(varName):
            if not isValidVarName(name):
                continue
            if name in self.interpreter.globalsDict:
                continue
            if self.searchVarName(name):
                continue
            self.vars[name] = varDecl
            if varDecl is not None:
                self.varNames[id(varDecl)] = name
    ...
    def registerLocalTypedef(self, typedef):
        ...
        if typedef in self.localTypes:
            return
        varName = self.registerNewUnscopedVarName(
            typedef.name or "anon_type", initNone=False)
    ...
    def getAstNodeForVarDecl(self, varDecl):
        ...
        assert name is not None, str(
            varDecl) + " is expected to be a global var"
...
class DidNotFindCTypesBasicType(Exception):
    pass
...
def getAstNodeForCTypesBasicType(t):
    ...
    if t is CVoidType:
        return NoneAstNode
    if not inspect.isclass(t) and isinstance(t, CVoidType):
        return NoneAstNode
    if inspect.isclass(t) and issubclass(t, CVoidType):
        return None
    if not inspect.isclass(t):
        raise DidNotFindCTypesBasicType("not a class")
    if issubclass(t, ctypes._Pointer):
        base_type = t._type_
        a = getAstNodeAttrib("ctypes", "POINTER")
        return makeAstNodeCall(a, getAstNodeForCTypesBasicType(base_type))
    if not issubclass(t, ctypes._SimpleCData):
        raise DidNotFindCTypesBasicType("unknown type")
    t_name = t.__name__
    if t_name.startswith("wrapCTypeClass_"):
        t_name = t_name[len("wrapCTypeClass_"):]
...
def getAstNodeForVarType(funcEnv, t):
    ...
    if isinstance(t, CBuiltinType):
        ...
        if t.name is None:
            ...
            v = getAstForWrapValue(interpreter, CWrapValue(
                getCType(t, interpreter.globalScope.stateStruct)))
        ...
        try:
            return getAstNodeForCTypesBasicType(t)
        except DidNotFindCTypesBasicType:
            pass
...
def _makeVal(funcEnv, f_arg_type, s_arg_ast, s_arg_type):
    ...
    if isinstance(s_arg_type, (tuple, list)):  # CCurlyArrayArgs
        ...
        for _f_arg_type, _s_arg_ast, _s_arg_type in zip(f_args, s_arg_ast.elts, s_arg_type):
            _s_arg_ast = _makeVal(funcEnv, _f_arg_type,
                                  _s_arg_ast, _s_arg_type)
            s_args += [_s_arg_ast]
    ...
    if use_value:
        ...
        if isinstance(s_arg_type, CWrapFuncType):
            ...
            need_cast = True
        if need_cast:
            s_arg_ast = getAstNode_newTypeInstance(
                funcEnv, f_arg_type, s_arg_ast, s_arg_type)
    return s_arg_ast
...
def getAstNode_newTypeInstance(funcEnv, objType, argAst=None, argType=None):
    ...
    if isinstance(objType, CArrayType):
        ...
        if objType.arrayLen:
            arrayLen = getConstValue(
                interpreter.globalScope.stateStruct, objType.arrayLen)
            ...
            if isinstance(argType, (tuple, list)):
                ...
                arrayLen = getConstValue(
                    interpreter.globalScope.stateStruct, argType.arrayLen)
                assert arrayLen is not None
            # Write back to type so that future getCType calls will succeed.
            objType.arrayLen = CNumber(arrayLen)

        typeAst = ast.BinOp(left=arrayOf, op=ast.Mult(),
                            right=ast.Num(n=arrayLen))
    else:
    ...
    if isinstance(argType, CWrapFuncType):
        if isVoidPtrType(objType):
            vAst = getAstNode_newTypeInstance(
                funcEnv, CFuncPointerDecl(
                    type=argType.func.type, args=argType.func.args),
                argAst=argAst, argType=argType)
            astCast = getAstNodeAttrib("ctypes", "cast")
            return makeAstNodeCall(astCast, vAst, typeAst)
        if isinstance(objType, CWrapFuncType):
            return argAst
        # what other case could there be?
        assert isinstance(objType, CFuncPointerDecl)
    ...
    if argAst is not None:
        if isinstance(argAst, (ast.Str, ast.Num)):
            ...
            args += [getAstNode_valueFromObj(
                interpreter._cStateWrapper, argAst, argType)]
        else:
...
class FuncCodeblockScope:
    ...
    def registerNewVar(self, varName, varDecl):
        ...
        if varDecl is None:
            ...
            a.value = getAstNode_newTypeInstance(
                self.funcEnv, varDecl.type, ast.Name(id=varName, ctx=ast.Load()), varDecl.type)
        elif isinstance(varDecl, CVarDecl):
            if varDecl.body is not None:
                bodyAst, t = astAndTypeForStatement(self.funcEnv, varDecl.body)
                v = getConstValue(
                    self.funcEnv.globalScope.stateStruct, varDecl.body)
                if v is not None and not v:
                    # If we want to init with 0, we can skip this because we are always zero initialized.
                    bodyAst = t = None
                a.value = getAstNode_newTypeInstance(
                    self.funcEnv, varDecl.type, bodyAst, t)
            else:
                a.value = getAstNode_newTypeInstance(
                    self.funcEnv, varDecl.type)
...
OpBinFuncsByOp = dict(
    map(lambda op: (op, _astOpToFunc(op)), OpBin.itervalues()))
...
class Helpers:
    ...
    def augAssign(a, op, bValue):
        if isinstance(a, (ctypes.c_void_p, ctypes._SimpleCData)):
            ...
            assert False, "augAssign: not handled: %r of type %r" % (
                a, type(a))
        return a
...
def autoCastArgs(funcEnv, required_arg_types, stmnt_args):
    ...
    required_arg_types = required_arg_types + \
        [None] * (len(stmnt_args) - len(required_arg_types))
    r_args = []
...
def astAndTypeForStatement(funcEnv, stmnt):
    if isinstance(stmnt, (CVarDecl, CFuncArgDecl)):
        ...
        if t is None:
            t = "int64_t"  # it's an overflow; just take a big type
        t = CStdIntType(t)
        return getAstNode_newTypeInstance(funcEnv, t, ast.Num(n=stmnt.content)), t
        ...
        ss = makeAstNodeCall(getAstNodeAttrib(
            "intp", "_make_string"), ast.Str(s=s))
        return ss, ta
        ...
        if isinstance(stmnt.base, CFunc):
            ...
            a.args = autoCastArgs(
                funcEnv, [f_arg.type for f_arg in stmnt.base.args], stmnt.args)
            ...
            if isinstance(a, CStatement) and not a.isCType():
                ...
                sizeValueAst = makeAstNodeCall(
                    getAstNodeAttrib("ctypes", "sizeof"), v)
                sizeAst = makeAstNodeCall(getAstNodeAttrib(
                    "ctypes_wrapped", "c_size_t"), sizeValueAst)
            ...
            s = ctypes.sizeof(t)
            sizeAst = makeAstNodeCall(getAstNodeAttrib(
                "ctypes_wrapped", "c_size_t"), ast.Num(s))
            return sizeAst, CStdIntType("size_t")
        elif isinstance(stmnt.base, CWrapValue):
            # expect that we just wrapped a callable function/object
            a = ast.Call(keywords=[], starargs=None, kwargs=None)
            a.func = getAstNodeAttrib(getAstForWrapValue(
                funcEnv.globalScope.interpreter, stmnt.base), "value")
            if isinstance(stmnt.base.value, ctypes._CFuncPtr):
                ...
                a.args = map(lambda arg: astAndTypeForStatement(
                    funcEnv, arg)[0], stmnt.args)
            returnType = stmnt.base.getReturnType(
                funcEnv.globalScope.stateStruct, stmnt.args)
            return a, returnType
            ...
            if isPointerType(pType.type, alsoArray=False) and not isVoidPtrType(pType.type):
                ...
                a = getAstNode_newTypeInstance(
                    funcEnv, pType.type, a, fixedReturnType)
            return a, pType.type
...
def getAstNode_assign(stateStruct, aAst, aType, bAst, bType):
    ...
    bValueAst = getAstNode_valueFromObj(
        stateStruct, bAst, bType, isPartOfCOp=True)
...
def getAstNode_ptrSubstract(stateStruct, aAst, aType, bAst, bType):
    ...
    aValueAst = getAstNode_valueFromObj(
        stateStruct, aAst, aType, isPartOfCOp=True)
    bValueAst = getAstNode_valueFromObj(
        stateStruct, bAst, bType, isPartOfCOp=True)
    ...
    resultAst = makeAstNodeCall(getAstNodeAttrib(
        "ctypes_wrapped", "c_long"), divAst)
    return resultAst


def _resolveSingleStatement(stmnt):
    if not isinstance(stmnt, CStatement):
        return stmnt
...
def _getZeroPtrTypeOrNone(stmnt):
    ...
    if not isinstance(stmnt, CFuncCall):
        return
    base = stmnt.base
    if isinstance(base, CStatement):
        if not base.isCType():
            return
        base = base.asType()
    if not isinstance(base, CPointerType):
        return
    assert len(stmnt.args) == 1
    arg = stmnt.args[0]
    assert isinstance(arg, CStatement)
    arg = _resolveSingleStatement(arg)
    if not isinstance(arg, CNumber):
        return
    if arg.content != 0:
        return
...
def _resolveOffsetOf(stateStruct, stmnt):
    if stmnt._leftexpr is not None:
        return
    ...
    if not isinstance(rightexpr, CPtrAccessRef):
        return
    zero_ptr_type = _getZeroPtrTypeOrNone(rightexpr.base)
    if zero_ptr_type is None:
        return
    attrib_chain = [rightexpr.name] + attrib_chain
    offset = 0
    base = zero_ptr_type
...
def makeFuncPtrValue(argAst, argType):
    ...
    v = getAstNode_newTypeInstance(
        argType.funcEnv, CBuiltinType(("void", "*")), argAst, argType)
...
def astAndTypeForCStatement(funcEnv, stmnt):
    ...
    if stmnt._leftexpr is None:  # prefixed only
        ...
        if stmnt._op.content == "++":
            ...
            if isinstance(rightType, CPointerType):
                ...
                # we cannot really dereference a funcptr with ctypes ...
    ...
    if stmnt._rightexpr is None:
        leftAstNode, leftType = astAndTypeForStatement(
            funcEnv, stmnt._leftexpr)
        if stmnt._op.content == "++":
    ...
    if stmnt._op.content == "=":
        ...
        a.values = [
            getAstNode_valueFromObj(
                funcEnv.globalScope.stateStruct, leftAstNode, leftType, isPartOfCOp=True),
        ...
        a.left = getAstNode_valueFromObj(
            funcEnv.globalScope.stateStruct, leftAstNode, leftType, isPartOfCOp=True)
        a.comparators = [getAstNode_valueFromObj(
            funcEnv.globalScope.stateStruct, rightAstNode, rightType, isPartOfCOp=True)]
        return getAstNode_newTypeInstance(funcEnv, ctypes.c_int, a), ctypes.c_int
    elif stmnt._op.content == "?:":
        middleAstNode, middleType = astAndTypeForStatement(
            funcEnv, stmnt._middleexpr)
        commonType = getCommonValueType(
            funcEnv.globalScope.stateStruct, middleType, rightType)
        a = ast.IfExp()
        a.test = getAstNode_valueFromObj(
            funcEnv.globalScope.stateStruct, leftAstNode, leftType, isPartOfCOp=True)
        a.body = getAstNode_newTypeInstance(
            funcEnv, commonType, middleAstNode, middleType)
        a.orelse = getAstNode_newTypeInstance(
            funcEnv, commonType, rightAstNode, rightType)
        return a, commonType
    elif stmnt._op.content == ",":
        ...
        a.left = getAstNode_valueFromObj(
            funcEnv.globalScope.stateStruct, leftAstNode, leftType, isPartOfCOp=True)
        a.right = getAstNode_valueFromObj(
            funcEnv.globalScope.stateStruct, rightAstNode, rightType, isPartOfCOp=True)
...
def astForCWhile(funcEnv, stmnt):
    ...
    whileAst.test = getAstNode_valueFromObj(
        funcEnv.globalScope.stateStruct, *astAndTypeForCStatement(funcEnv, stmnt.args[0]), isPartOfCOp=True)

    funcEnv.pushScope(whileAst.body)
    if stmnt.body is not None:
        cCodeToPyAstList(funcEnv, stmnt.body)
    if not whileAst.body:
        whileAst.body.append(ast.Pass())
    funcEnv.popScope()
...
def astForCFor(funcEnv, stmnt):
    ...
    assert isinstance(stmnt.args[1], CStatement)

    # introduce dummy 'if' AST so that we have a scope for the for-loop (esp. the first statement)
    ifAst = ast.If(body=[], orelse=[], test=ast.Name(
        id="True", ctx=ast.Load()))
    ...
    ifFirstIterAst = ast.If(body=[], orelse=[], test=ast.Name(
        id=var_first_iteration, ctx=ast.Load()))
    a = ast.Assign()
    a.targets = [ast.Name(id=var_first_iteration, ctx=ast.Store())]
    ...
    if stmnt.args[1]:  # non-empty statement
        ...
        ifTestAst.test = getAstNode_valueFromObj(
            funcEnv.globalScope.stateStruct, *astAndTypeForCStatement(funcEnv, stmnt.args[1]), isPartOfCOp=True)
...
def astForCDoWhile(funcEnv, stmnt):
    ...
    ifAst.test = getAstNode_valueFromObj(
        funcEnv.globalScope.stateStruct, *astAndTypeForCStatement(funcEnv, stmnt.whilePart.args[0]), isPartOfCOp=True)
...
def astForCSwitch(funcEnv, stmnt):
    ...
    switchValueAst, switchValueType = astAndTypeForCStatement(
        funcEnv, stmnt.args[0])
    a = ast.Assign()
    a.targets = [ast.Name(id=switchVarName, ctx=ast.Store())]
    a.value = getAstNode_valueFromObj(
        funcEnv.globalScope.stateStruct, switchValueAst, switchValueType, isPartOfCOp=True)
        ...
        if isinstance(c, CCaseStatement):
            ...
            curCase.test = ast.BoolOp(op=ast.Or(), values=[
                ...
                ast.Compare(
                    ...
                    ops=[ast.Eq()],
                    comparators=[getAstNode_valueFromObj(
                        funcEnv.globalScope.stateStruct, *astAndTypeForCStatement(funcEnv, c.args[0]), isPartOfCOp=True)]
                )
            ])
...
def astForCReturn(funcEnv, stmnt):
    ...
    if stmnt is None:
        ...
        returnValueAst = getAstNode_valueFromObj(
            funcEnv.globalScope.stateStruct, emptyAst, funcEnv.func.type)
    else:
        ...
        if returnValueAst is None:
            ...
            if isPointerType(valueType):
                valueAst = makeAstNodeCall(
                    getAstNodeAttrib("intp", "_storePtr"), valueAst)
            returnValueAst = getAstNode_valueFromObj(
                funcEnv.globalScope.stateStruct, valueAst, valueType)
...
class Interpreter:
    def __init__(self):
        ...
        self.globalsStructWrapper = GlobalsTypeWrapper(
            self.globalScope, "structs")
        self.globalsUnionsWrapper = GlobalsTypeWrapper(
            self.globalScope, "unions")
    ...
    def _realloc(self, ptr_addr, size):
        ...
        try:
            ...
            raise Exception(
                "_realloc: address 0x%x was not allocated by us" % ptr_addr)
        if buf._length_ >= size:
            return ctypes.cast(buf, wrapCTypeClass(ctypes.c_void_p))
        ptr = self._malloc(size)
        ctypes.memmove(ptr, ctypes.cast(buf, wrapCTypeClass(
            ctypes.c_void_p)), ctypes.c_size_t(buf._length_))
        return ptr
    ...
    def _free(self, ptr_addr):
        try:
            ...
            raise Exception(
                "_free: address 0x%x was not allocated by us" % ptr_addr)

    def _storePtr(self, ptr, offset=0, value=None):
        assert isinstance(
            ptr, (ctypes.c_void_p, ctypes._Pointer, ctypes.Array, ctypes._CFuncPtr))
        ...
        assert not isinstance(ptr, ctypes._CFuncPtr)
        ...
        for obj_ptr_addr, obj_size in self.pointerStorageRanges.irange(
            ...
            if obj_ptr_addr + obj_size <= ptr_addr:
                break
    ...
    def _getPtr(self, addr, ptr_type=None):
        ...
        try:
            ...
            raise Exception(
                "invalid pointer access to address 0x%x of type %r" % (addr, ptr_type))
    ...
    def _translateFuncToPyAst(self, func, noBodyMode="warn-empty"):
        ...
        for arg in func.args:
            if isinstance(arg.type, CVariadicArgsType):
                name = base.registerNewUnscopedVarName(
                    "varargs", initNone=False)
                assert name
                base.astNode.args.vararg = name
            else:  # normal param
                name = base.registerNewVar(arg.name, arg)
                assert name
                base.astNode.args.args.append(
                    ast.Name(id=name, ctx=ast.Param()))
        ...
        if base.needGotoHandling:
            gotoVarName = base.registerNewUnscopedVarName(
                "goto", initNone=False)
    ...
    def _translateFuncToPy(self, funcname):
        ...
        if self.debug_print_getFunc:
            print("+ getFunc %s" % cfunc)
    ...
    def runFunc(self, funcname, *args, **kwargs):
        ...
        args = map(lambda (arg, typ): self._castArgToCType(
            arg, typ), zip(args, f.C_argTypes))
        res = f(*args)
