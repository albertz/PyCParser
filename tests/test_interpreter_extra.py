...
def test_file_scope_statics_are_isolated_per_translation_unit():
    """Two .c files each declare ``static T p;`` with incompatible
    types.  Per ISO C this is fine -- file-scope ``static`` has
    internal linkage, so the two ``p``s are SEPARATE variables.

    cparser handles this by mangling each file's file-scope statics
    with a ``_static_<basename>_`` prefix at the end of ``cparser.parse``
    (see ``_rename_tu_statics``).  This test verifies the isolation: no
    type-confusion error fires, both statics live in ``state.vars``
    under distinct mangled names, and within-TU references still
    resolve correctly (they hold object pointers that survive the
    rename).
    """
    import os
    import tempfile
    ...
    coll = [e for e in state._errors if "TYPE-CONFUSION" in e or "collision across translation units" in e]
    assert not coll, "auto-scoping should prevent any collision, got: %r" % coll

    assert "_static_a_p" in state.vars
    assert "_static_b_p" in state.vars
    assert state.vars["_static_a_p"] is not state.vars["_static_b_p"]

    # Within-TU reference survives the rename: set_p (non-static) holds
    # a pointer to file A's `p`, whose .name now reflects the mangle.
    set_p = state.funcs["set_p"]
    assert set_p is not None
    assert state.vars["_static_a_p"].name == "_static_a_p"
    assert state.vars["_static_b_p"].name == "_static_b_p"
