...
def test_file_scope_static_collision_resolved_by_auto_scoping():
    """Two .c files each declare ``static T *shared;`` with DIFFERENT
    types.  Per ISO C these are SEPARATE variables (internal linkage).
    ``cparser.parse`` auto-mangles file-scope statics with a
    ``_static_<basename>_`` prefix, so the two end up as distinct
    entries -- no collision, no error.
    """
    src_a = """
    ...
    collisions = [e for e in state._errors
                  if "shared" in e and ("MEMORY CORRUPTION" in e
                                        or "collision across translation units" in e)]
    assert not collisions, (
        "auto-scoping should isolate per-TU statics; got: %r" % collisions)
    assert "_static_a_shared" in state.vars
    assert "_static_b_shared" in state.vars
    assert state.vars["_static_a_shared"] is not state.vars["_static_b_shared"]


def test_file_scope_static_same_name_same_type_auto_isolated():
    """Same-name same-type statics across TUs (eg. per-file
    ``static char doc[]``) end up as distinct ``state.vars`` entries
    after auto-scoping -- each file gets its own variable.
    """
    src_a = """
    ...
    """
    state = _write_two_files_and_parse(src_a, src_b)
    collisions = [e for e in state._errors if "doc" in e and "collision" in e.lower()]
    assert not collisions, "auto-scoping should isolate per-TU statics; got: %r" % collisions
    assert "_static_a_doc" in state.vars
