        ...
        for c in input:
            if state == 0:
                ...
                if c in NumberChars:
                    ...
                    pass  # even if invalid, stay in this state
                else:
                    state = 0
            elif state == 3:  # hex number
                ret += c
                if c in NumberChars + LetterChars + "_":
                    pass  # also ignore invalids
                else:
                    state = 0
            elif state == 4:  # str
                ret += c
                ...
                if c in SpaceChars + LetterChars + "_":
                    ...
                    stateStruct.error(
                        "unfold macro: unexpected char '" + c + "' after #")
                    state = 0
                ...
                if c in LetterChars + NumberChars + "_":
                    ...
                    if lastidentifier not in args:
                        stateStruct.error(
                            "unfold macro: cannot stringify " + lastidentifier + ": not found")
                    else:
                ...
                if c == "#":
                    ...
                    stateStruct.error(
                        "unfold macro: unexpected char %r after in state %i" % (c, state))
                    state = 0  # recover
                ...
                stateStruct.error(
                    "unfold macro: internal error, char %r, in state %i" % (c, state))
                state = 0  # recover
        # Final check.
        if state == 1:
            if lastidentifier in args:
...
class Macro(object):
    ...
    def eval(self, state, args):
        if len(args) != len(self.args or ()):
            raise TypeError("invalid number of args (" +
                            str(args) + ") for " + repr(self))
    ...
    def getCValue(self, stateStruct):
        ...
        for token in input_iter:
            if isinstance(token, COpeningBracket):
                valueStmnt._cpre3_parse_brackets(
                    stateStruct, token, input_iter)
            else:
...
class CType(object):
    ...
    def __hash__(self): return hash(self.__class__) + 31 * \
...
class CPointerType(CType):
    ...
    def getCType(self, stateStruct):
        try:
            ...
            stateStruct.error("getCType " + str(self) +
                              ": error getting type (" + str(e) + "), falling back to void-ptr")
...
class CArrayType(CType):
    ...
    def getCType(self, stateStruct):
        try:
            ...
            stateStruct.error(
                str(self) + ": error getting type (" + str(e) + "), falling back to int")
            t = ctypes.c_int
        ...
        if l is None:
            stateStruct.error(
                "%s: error getting array len, falling back to 1" % self)
            l = 1
        return t * l

    def asCCode(self, indent=""): return "%s%s[%s]" % (
        indent, asCCode(self.arrayOf), asCCode(self.arrayLen))
...
def isType(t):
    if isinstance(t, CType) and not isinstance(t, CWrapValue):
        return True
    ...
    try:
        ...
        pass  # e.g. typeerror or so
    if isinstance(t, (CStruct, CUnion, CEnum, CTypedef)):
        return True
    return False
...
class State(object):
    ...
    StdIntTypes = {
        ...
        # NOTE: not really correct but shouldn't matter unless we directly access it
        "FILE": ctypes.c_int,
    }
    Attribs = [
        "const",
    ...
    def getDictNameForType(cls, objType):
        if issubclass(objType, Macro):
            return "macros"
        if issubclass(objType, CTypedef):
            return "typedef"  # not really consistent
        if issubclass(objType, CStruct):
            return "structs"
        if issubclass(objType, CUnion):
            return "unions"
        if issubclass(objType, CEnum):
            return "enums"
        if issubclass(objType, CFunc):
            return "funcs"
        if issubclass(objType, CVarDecl):
            return "vars"
        if issubclass(objType, CEnumConst):
            return "enumconsts"
    ...
    def autoSetupSystemMacros(self, system_specific=False):
        ...
        # most headers just behave more sane with this :)
        self.macros["__GNUC__"] = Macro(rightside="4")
        ...
        if system_specific and sys.platform == "darwin":
            ...
            self.macros["MAC_OS_X_VERSION_MIN_REQUIRED"] = Macro(
                rightside="1030")

    def autoSetupGlobalIncludeWrappers(self):
        if self._global_include_wrapper:
            return
    ...
    def incIncludeLineChar(self, fullfilename=None, inc=None, line=None, char=None, charMod=None):
        ...
        LineStartIndex = 1
        if inc is not None:
            self._preprocessIncludeLevel += [[fullfilename,
                                              inc, LineStartIndex, CharStartIndex]]
        if len(self._preprocessIncludeLevel) == 0:
            self._preprocessIncludeLevel += [[None,
                                              "<input>", LineStartIndex, CharStartIndex]]
        if line is not None:
    ...
    def curFile(self):
        if not self._preprocessIncludeLevel:
            return "<out-of-scope>"
    ...
    def readLocalInclude(self, filename):
        ...
        try:
            ...
            self.error("cannot open local include-file '" +
                       filename + "': " + str(e))
            return "", None
    ...
    def readGlobalInclude(self, filename):
        if filename == "inttypes.h":
            return "", None  # we define those types as builtin-types
        elif filename == "stdint.h":
            return "", None
        else:
...
def cpreprocess_evaluate_single(state, arg):
    if arg == "":
        return None
    try:
        return int(arg)  # is integer?
    except ValueError:
        pass
    try:
        return long(arg)  # is long?
    except ValueError:
        pass
    try:
        return int(arg, 16)  # is hex?
    except ValueError:
        pass
    if len(arg) >= 2 and arg[0] == '"' and arg[-1] == '"':
        return arg[1:-1]  # is string?

    if not is_valid_defname(arg):
        state.error("preprocessor eval single: '" +
                    arg + "' is not a valid macro name")
        return 0
    ...
    try:
        ...
        state.error("preprocessor eval single error on '" +
                    arg + "': " + str(e))
        return 0
...
def cpreprocess_evaluate_cond(stateStruct, condstr):
        ...
        while not breakLoop:
            ...
            if state == 0:
                if c == "(":
                    ...
                    stateStruct.error(
                        "preprocessor: runaway ')' in " + repr(condstr))
                    return
                ...
                if c == ")":
                    bracketLevel -= 1
                    if bracketLevel == 0:
                        neweval = cpreprocess_evaluate_cond(
                            stateStruct, substr)
                        state = 18
                ...
                if c in SpaceChars:
                    ...
                    stateStruct.error(
                        "preprocessor eval: 'defined' invalid in '" + condstr + "'")
                    return
                else:
                    laststr = c
                    state = 6
                ...
                if c in LetterChars + "_" + NumberChars:
                    ...
                    if not is_valid_defname(macroname):
                        stateStruct.error(
                            "preprocessor eval defined-check: '" + macroname + "' is not a valid macro name")
                        return
                ...
                if c in SpaceChars:
                    ...
                    if laststr != "":
                        neweval = cpreprocess_evaluate_single(
                            stateStruct, laststr)
                        if prefixOp is not None:
                            neweval = prefixOp(neweval)
                            prefixOp = None
                    ...
                    stateStruct.error("preprocessor eval: '" + c + "' not expected after '" +
                                      laststr + "' in state 10 with '" + condstr + "'")
                    return
                ...
                if c == "(":
                    ...
                    if macroname == "defined":
                        if len(args) != 1:
                            stateStruct.error(
                                "preprocessor eval defined-check args invalid: " + str(args))
                            return
                        else:
                            macroname = args[0]
                            if not is_valid_defname(macroname):
                        ...
                        if not is_valid_defname(macroname):
                            stateStruct.error(
                                "preprocessor eval call: '" + macroname + "' is not a valid macro name in " + repr(condstr))
                            return
                        if macroname not in stateStruct.macros:
                            stateStruct.error(
                                "preprocessor eval call: '" + macroname + "' is unknown")
                            return
                        ...
                        try:
                            ...
                            stateStruct.error(
                                "preprocessor eval call on '" + macroname + "': error " + str(e))
                            return
                        neweval = cpreprocess_evaluate_cond(
                            stateStruct, resolved)
                ...
                if c in OpChars:
                    ...
                    if opstr == "":
                        if c in SpaceChars:
                            ...
                            stateStruct.error(
                                "preprocessor eval: expected op but got '" + c + "' in '" + condstr + "' in state 18")
                            return
                    else:
                        if opstr == "&&":
                            def op(x, y): return x and y
                            # short path check
                            if not lasteval:
                                return lasteval
                        elif opstr == "||":
                            def op(x, y): return x or y
                            # short path check
                            if lasteval:
                                return lasteval
                            ...
                            if prefixOp:
                                def prefixOp(x): return prefixOp(
                                    newprefixop(x))
                            else:
                                prefixOp = newprefixop
                        else:
                            stateStruct.error(
                                "invalid op '" + opstr + "' with '" + c + "' following in '" + condstr + "'")
                            return
                        opstr = ""
                        laststr = ""
                        state = 0
                ...
                stateStruct.error(
                    "internal error in preprocessor evaluation: state " + str(state))
                return
    ...
    if state in (0, 10):
        ...
        macroname = laststr
        if not is_valid_defname(macroname):
            stateStruct.error("preprocessor eval defined-check: '" +
        ...
        if opstr != "":
            stateStruct.error(
                "preprocessor eval: unfinished op: '" + opstr + "'")
        else:
            pass
    else:
        stateStruct.error("preprocessor eval: invalid argument: '" +
                          condstr + "'. unfinished state " + str(state))
...
def cpreprocess_handle_include(state, arg):
    ...
    if arg[0] == '"' and arg[-1] == '"':
        ...
        state.error("invalid include argument: '" + arg + "'")
        return
    for c in state.preprocess_file(filename=filename, local=local):
        yield c
...
def cpreprocess_handle_def(stateStruct, arg):
    ...
    if not is_valid_defname(macroname):
        stateStruct.error("preprocessor define: '" +
                          macroname + "' is not a valid macro name")
        return
...
def handle_cpreprocess_cmd(state, cmd, arg):
    ...
    if cmd == "ifdef":
        ...
        if state._preprocessIgnoreCurrent:
            return
        for c in cpreprocess_handle_include(state, arg):
            yield c
    ...
    state._preprocessIgnoreCurrent = any(
        map(lambda x: x != 1, state._preprocessIfLevels))
        ...
        while not breakLoop:
            ...
            if state == 0:
                ...
                if c in SpaceChars:
                    ...
                    for c in handle_cpreprocess_cmd(stateStruct, cmd, arg):
                        yield c
                    state = 0
                else:
                    if arg is None:
                ...
                if c == "\n":
                    ...
                    pass  # ignore everything, wait for newline
            elif state == 10:  # after '"'
                if not stateStruct._preprocessIgnoreCurrent:
                    yield c
                if c == "\\":
                ...
                if c == "*":
                    state = 21  # C-style comment
                elif c == "/":
                    state = 25  # C++-style comment
                else:
                    ...
                    if state == 0:
                        ...
                        stateStruct.error(
                            "preproc parse: internal error after possible comment. didn't expect state " + str(state))
                ...
                stateStruct.error(
                    "internal error: invalid state " + str(state))
                state = 0  # reset. it's the best we can do

        if c == "\n":
            stateStruct.incIncludeLineChar(line=1)
        elif c == "\t":
            stateStruct.incIncludeLineChar(char=4, charMod=4)
        else:
...
class CStr(_CBase):
    ...
    def asCCode(self, indent=""): return indent + \
        '"' + escape_cstr(self.content) + '"'


class CChar(_CBase):
    def __init__(self, content=None, rawstr=None, **kwargs):
        if isinstance(content, (unicode, str)):
            content = ord(content)
        assert isinstance(
            content, int), "CChar expects int, got " + repr(content)
        assert 0 <= content <= 255, "CChar expects number in range 0-255, got " + \
            str(content)
...
class COpeningBracket(_CBase):
    pass


class CClosingBracket(_CBase):
    pass
...
def cpre2_parse_number(stateStruct, s):
    if len(s) > 1 and s[0] == "0" and s[1] in NumberChars:
        try:
            ...
            stateStruct.error("cpre2_parse_number: " + s +
                              " looks like octal but got error " + str(e))
            return 0
    if len(s) > 1 and s[0] == "0" and s[1] in "xX":
        try:
            ...
            stateStruct.error("cpre2_parse_number: " + s +
                              " looks like hex but got error " + str(e))
            return 0
    try:
        s = s.rstrip("ULul")
        return long(s)
            ...
            if state == 0:
                if c in SpaceChars + "\n":
                    ...
                    if len(brackets) == 0 or ClosingBrackets[len(OpeningBrackets) - OpeningBrackets.index(brackets[-1]) - 1] != c:
                        stateStruct.error(
                            "cpre2 parse: got '" + c + "' but bracket level was " + str(brackets))
                    else:
                        brackets[:] = brackets[:-1]
                    ...
                    stateStruct.error(
                        "cpre2 parse: didn't expected char %r in state %i" % (c, state))
                ...
                state = 0
            # number (no correct float handling, will be [number, op("."), number])
            elif state == 10:
                if c in NumberChars:
                    laststr += c
                elif c in LetterChars + "_":
                ...
                if c == "'":
                    # hacky check for '\0abc'-like strings.
                    if len(laststr) > 1 and laststr[0] == '\0':
                ...
                if c in SpaceChars + "\n":
                    ...
                    if c != "(":
                        ...
                        macroargs = _cpre2_parse_args(
                            stateStruct, input, brackets=brackets + [c])
                        state = 32
                ...
                try:
                    resolved = stateStruct.macros[macroname].eval(
                        stateStruct, macroargs)
                except Exception as e:
                    stateStruct.error(
                        "cpre2 parse unfold macro " + macroname + " error: " + repr(e))
                    resolved = ""
                ...
                stateStruct.error(
                    "cpre2 parse: internal error. didn't expected state " + str(state))
...
def make_type_from_typetokens(stateStruct, curCObj, type_tokens):
    ...
    if len(type_tokens) == 1 and isinstance(type_tokens[0], _CBaseWithOptBody):
        ...
        t = CPointerType(make_type_from_typetokens(
            stateStruct, curCObj, type_tokens[:-1]))
...
def asCCode(stmnt, indent="", fullDecl=False):
    if not fullDecl:
        if isinstance(stmnt, CFunc):
            return indent + stmnt.name
        if isinstance(stmnt, CStruct):
            return indent + "struct " + stmnt.name
        if isinstance(stmnt, CUnion):
            return indent + "union " + stmnt.name
        if isinstance(stmnt, CEnum):
            return indent + "enum " + stmnt.name
...
class _CBaseWithOptBody(object):
    ...
    def __str__(self):
        ...
        for attrName, addCheck, displayName, displayFunc in self.StrOutAttribList:
            ...
            if addCheck is None:
                def addCheck(x): return x is not None
            if addCheck(a):
    ...
    def finalize(self, stateStruct, addToContent=None):
        if self._finalized:
            stateStruct.error("internal error: " +
                              str(self) + " finalized twice")
            return
        ...
        if addToContent is None:
            addToContent = self.AutoAddToContent
    ...
    def _copy(self, value, parent=None, name=None, leave_out_attribs=()):
        if isinstance(value, (int, long, float, str, unicode)) or value is None:
            ...
            assert False, "dont know how to handle %r %r (%s)" % (
...
class CTypedef(_CBaseWithOptBody):
    def finalize(self, stateStruct):
        ...
        self.type = make_type_from_typetokens(
            stateStruct, self, self._type_tokens)
        _CBaseWithOptBody.finalize(self, stateStruct)

        if self.type is None:
            stateStruct.error("finalize typedef " +
                              str(self) + ": type is unknown")
            return
        if self.name is None:
            stateStruct.error("finalize typedef " +
                              str(self) + ": name is unset")
            return
...
class CFuncPointerDecl(_CBaseWithOptBody, CFuncPointerBase):
    ...
    def getCType(self, stateStruct, workaroundPtrReturn=True, wrap=True):
        ...
        if wrap:
            restype = wrapCTypeClassIfNeeded(restype)
        argtypes = map(lambda a: getCType(a, stateStruct), self.args)
        if wrap:
            argtypes = map(wrapCTypeClassIfNeeded, argtypes)
...
def _finalizeBasicType(obj, stateStruct, dictName=None, listName=None, addToContent=None, allowPredec=True):
    ...
    if obj.type is None:
        obj.type = make_type_from_typetokens(
            stateStruct, obj, obj._type_tokens)
    ...
    if addToContent and hasattr(obj.parent, "body") and not getattr(obj, "_already_added", False):
        _addToParent(obj=obj, stateStruct=stateStruct, dictName=dictName,
                     listName=listName, allowPredec=allowPredec)


class CFunc(_CBaseWithOptBody):
    finalize = lambda *args, **kwargs: _finalizeBasicType(
        *args, dictName="funcs", **kwargs)
...
class CVarDecl(_CBaseWithOptBody):
    finalize = lambda *args, **kwargs: _finalizeBasicType(
        *args, dictName="vars", allowPredec=False, **kwargs)

    def clearDeclForNextVar(self):
        if hasattr(self, "bitsize"):
            delattr(self, "bitsize")
...
class CTypeConstructionException(Exception):
    pass


class RecursiveStructConstruction(CTypeConstructionException):
    pass
...
def _getCTypeStruct(baseClass, obj, stateStruct):
    ...
    def _construct(obj):
        ...
        for c in obj.body.contentlist:
            ...
            if c.arrayargs:
                if len(c.arrayargs) != 1:
                    raise Exception(str(c) + " has too many array args")
    ...
    if getattr(obj, "_ctype_is_constructing", None):
        ...
        if obj._ctype_construct_need_now:
            raise RecursiveStructConstruction(
                "Recursive construction of type %s" % obj)
    ...
    if obj.body is None:
        raise CTypeConstructionException(
...
class CEnum(_CBaseWithOptBody):
    ...
    def getEnumConst(self, value):
        for c in self.body.contentlist:
            if not isinstance(c, CEnumConst):
                continue
            if c.value == value:
                return c
        return None
...
def _isBracketLevelOk(parentLevel, curLevel):
    if parentLevel is None:
        parentLevel = []
    if len(parentLevel) > len(curLevel):
        return False
...
def opsDoLeftToRight(stateStruct, op1, op2):
    try:
        opprec1 = OpPrecedences[op1]
    except KeyError:
        stateStruct.error(
            "internal error: statement parsing: op1 " + repr(op1) + " unknown")
        opprec1 = 100
    try:
        opprec2 = OpPrecedences[op2]
    except KeyError:
        stateStruct.error(
            "internal error: statement parsing: op2 " + repr(op2) + " unknown")
        opprec2 = 100
...
def getConstValue(stateStruct, obj):
    ...
    """
    if hasattr(obj, "getConstValue"):
        return obj.getConstValue(stateStruct)
    ...
    if isinstance(obj, CFuncCall):  # maybe a cast
        ...
        if isinstance(t, (CBuiltinType, CStdIntType)):  # only number types
            ...
            if v is None:
                return None  # cannot handle anyway
            ctype = getCType(t, stateStruct)
            if isIntType(obj.base):
                v = int(v)
            if v:
                cv = ctype(v)
            else:
...
def getValueType(stateStruct, obj):
    if hasattr(obj, "getValueType"):
        return obj.getValueType(stateStruct)
    ...
    if isinstance(obj, CNumber):
        ...
        t = minCIntTypeForNums(obj.content, minBits=32,
                               maxBits=64, useUnsignedTypes=True)
...
def getCommonValueType(stateStruct, t1, t2):
    ...
    if isinstance(t1, CArrayType) or isinstance(t2, CArrayType):
        if isinstance(t1, CArrayType) and isinstance(t2, CArrayType):
            if isSameType(stateStruct, t1, t2):
                return t1
    ...
    if isinstance(t1, CBuiltinType) and isinstance(t2, CBuiltinType):
        ...
        if unsigned_t1:
            assert tup1[0] == "unsigned"
        if unsigned_t2:
            assert tup2[0] == "unsigned"
...
class CStatement(_CBaseWithOptBody):
    ...
    def __repr__(self):
        ...
        if self._leftexpr is not None:
            s += " " + repr(self._leftexpr)
        if self._op == COp("?:"):
    ...
    def _handlePushedErrorForUnknown(self, stateStruct):
        if isinstance(self._leftexpr, CUnknownType):
            ...
            if not s:
                stateStruct.error("statement parsing: identifier %r unknown in state %i in handle pushed error" % (
                    self._leftexpr.name, self._state))
    ...
    def _cpre3_handle_token(self, stateStruct, token):
        ...
        if self._state == 5 and token == COp(":"):
            if self._leftexpr.name:
                ...
                stateStruct.error("statement parsing: got ':' after " + repr(
                    self._leftexpr) + "; looks like a goto-label but has no name")
        ...
        if self._state == 0:
            if isinstance(token, (CIdentifier, CNumber, CStr, CChar)):
                if isinstance(token, CIdentifier):
                    if token.content == "struct":
                        ...
                        obj = findObjInNamespace(
                            stateStruct, self.parent, token.content)
                        if obj is None:
                ...
                stateStruct.error(
                    "statement parsing: didn't expected token " + str(token))
        elif self._state in (1, 2, 3):  # struct,union,enum
            TName = {1: "struct", 2: "union", 3: "enum"}[self._state]
            DictName = TName + "s"
            if isinstance(token, CIdentifier):
                obj = findCObjTypeInNamespace(
                    stateStruct, self.parent, DictName, token.content)
                if obj is None:
                    stateStruct.error("statement parsing: " +
                                      TName + " '" + token.content + "' unknown")
                    obj = CUnknownType(name=token.content)
                self._leftexpr = obj
                self._state = 5
            else:
                stateStruct.error(
                    "statement parsing: didn't expected token " + str(token) + " after " + TName)
            ...
            if token == COp("."):
                if isinstance(self._leftexpr, CNumber):
                    ...
                    self._state = 20
                    self._leftexpr = CAttribAccessRef(
                        parent=self, base=self._leftexpr)
            elif token == COp("->"):
                self._state = 20
                self._leftexpr = CPtrAccessRef(
                    parent=self, base=self._leftexpr)
            elif isinstance(token, COp):
                ...
                if isinstance(self._leftexpr, CBuiltinType) and self._leftexpr.builtinType + (token.content,) in stateStruct.CBuiltinTypes:
                    self._leftexpr = CBuiltinType(
                        self._leftexpr.builtinType + (token.content,))
                    # stay in same state
                else:
                    self._leftexpr = _create_cast_call(
                        stateStruct, self, self._leftexpr, token)
                    self._state = 40
            ...
            if isinstance(token, CIdentifier):
                if token.content == "sizeof":
                    ...
                    obj = findObjInNamespace(
                        stateStruct, self.parent, token.content)
                    if obj is None:
                        stateStruct.error("statement parsing: identifier %r unknown in state %i" % (
                            token.content, self._state))
                        obj = CUnknownType(name=token.content)
                self._state = 7
                ...
                # maybe a postfix op or whatever
                obj._cpre3_handle_token(stateStruct, token)
                self._state = 8
            self._rightexpr = obj
            ...
            if token == COp("."):
                if isinstance(self._rightexpr, CNumber):
                    ...
                    self._state = 22
                    self._rightexpr = CAttribAccessRef(
                        parent=self, base=self._rightexpr)
            elif token == COp("->"):
                self._state = 22
                self._rightexpr = CPtrAccessRef(
                    parent=self, base=self._rightexpr)
            elif isinstance(token, COp):
                if token == COp(":"):
                    if self._op != COp("?"):
                        stateStruct.error(
                            "internal error: got ':' after " + repr(self) + " with " + repr(self._op))
                    ...
                    self._rightexpr = CStatement(
                        parent=self, _leftexpr=self._rightexpr, _state=6)
                ...
                self._rightexpr = _create_cast_call(
                    stateStruct, self, self._rightexpr, token)
                self._state = 45
            ...
            if isinstance(token, CNumber):
                self._leftexpr = CNumber(
                    float("%s.%s" % (self._leftexpr.content, token.content)))
            else:
                stateStruct.error(
                    "statement parsing: did not expect %s in state %i" % (token, self._state))
            self._state = 5
        elif self._state == 11:  # after expr + op + number + "."
            if isinstance(token, CNumber):
                self._rightexpr = CNumber(
                    float("%s.%s" % (self._rightexpr.content, token.content)))
            else:
                stateStruct.error(
                    "statement parsing: did not expect %s in state %i" % (token, self._state))
            self._state = 7
            ...
            if isinstance(token, CIdentifier):
                assert isinstance(
                    self._leftexpr, (CAttribAccessRef, CPtrAccessRef))
                self._leftexpr.name = token.content
                self._state = 5
            else:
                stateStruct.error("statement parsing: didn't expected token " + str(
                    token) + " after " + str(self._leftexpr) + " in state " + str(self._state))
        elif self._state == 22:  # after expr + op + expr with attrib/ptr access
            if isinstance(token, CIdentifier):
                assert isinstance(
                    self._rightexpr, (CAttribAccessRef, CPtrAccessRef))
            ...
            # something is unfinished, like a previous "->"
            if self._leftexpr.args[0]._state != 5:
            ...
            # something is unfinished, like a previous "->"
            if self._rightexpr.args[0]._state != 5:
            ...
            stateStruct.error("internal error: statement parsing: token " +
                              str(token) + " in invalid state " + str(self._state))

    def _cpre3_parse_brackets(self, stateStruct, openingBracketToken, input_iter):
        self._handlePushedErrorForUnknown(stateStruct)

        # array args or struct args
        if self._state == 0 and openingBracketToken.content == "{":
            arrayArgs = CCurlyArrayArgs(parent=self)
            self._leftexpr = arrayArgs
            arrayArgs._bracketlevel = list(openingBracketToken.brackets)
            cpre3_parse_statements_in_brackets(
                stateStruct, arrayArgs, COp(","), arrayArgs.args, input_iter)
        ...
        if self._state in (50, 51):  # after [expr + op +] (expr)-cast
            ...
            if funcCall.args:
                ...
                funcCall.args[0]._cpre3_parse_brackets(
                    stateStruct, openingBracketToken, input_iter)
            else:
                ...
                subStatement._cpre3_parse_brackets(
                    stateStruct, openingBracketToken, input_iter)
                if subStatement._state == 50:
                    return  # another cast follows
                funcCall.finalize(stateStruct)
        ...
        if self._state in (5, 7):  # after expr or expr + op + expr
            ...
            if openingBracketToken.content == "(":
                ...
                stateStruct.error(
                    "cpre3 statement parse brackets after expr: didn't expected opening bracket '" + openingBracketToken.content + "'")
            ...
            funcCall._bracketlevel = list(openingBracketToken.brackets)
            cpre3_parse_statements_in_brackets(
                stateStruct, funcCall, COp(","), funcCall.args, input_iter)
            funcCall.finalize(stateStruct)
            return

        if self._state in (8, 9):  # right-to-left chain
            self._rightexpr._cpre3_parse_brackets(
                stateStruct, openingBracketToken, input_iter)
        ...
        if self._state in (40, 45):  # after .. cast_call + expr
            ...
            ref.args[0]._cpre3_parse_brackets(
                stateStruct, openingBracketToken, input_iter)
            return
        ...
        if self._state == 0:
            ...
            stateStruct.error("cpre3 statement parse brackets: didn't expected opening bracket '" +
                              openingBracketToken.content + "' in state " + str(self._state))
        ...
        for token in input_iter:
            if isinstance(token, COpeningBracket):
                ...
                if token.brackets == openingBracketToken.brackets:
                    ...
                    stateStruct.error(
                        "cpre3 statement parse brackets: internal error, closing brackets " + str(token.brackets) + " not expected")
            else:
                subStatement._cpre3_handle_token(stateStruct, token)
        if not finalized:
            stateStruct.error("cpre3 statement parse brackets: incomplete, missing closing bracket '" +
                              openingBracketToken.content + "' at level " + str(openingBracketToken.brackets))
            return
    ...
    def isCType(self):
        if self._leftexpr is None:
            return False  # all prefixed stuff is not a type
        if self._rightexpr is not None:
            return False  # same thing, prefixed stuff is not a type
    ...
    def asType(self):
        ...
        if self._op is not None:
            if self._op.content in ("*", "&"):
                ...
                raise Exception("postfix op " + str(self._op) +
                                " unknown for pointer type " + str(self._leftexpr))
        return t
    ...
    def asCCode(self, indent=""):
        ...
        if self._op is None or self._rightexpr is None:
            # no brackets. we do them outside
            return indent + asCCode(self._leftexpr)
        if self._op == COp("?:"):
...
def cpre3_parse_funcpointername(stateStruct, curCObj, input_iter):
    ...
    for token in input_iter:
        if isinstance(token, CClosingBracket):
            ...
            if not _isBracketLevelOk(bracketLevel, token.brackets):
                stateStruct.error(
                    "cpre3 parse func pointer name: internal error: bracket level messed up with closing bracket: " + str(token.brackets))
        ...
        if state == 0:
            if token == COp("*"):
                ...
                stateStruct.error("cpre3 parse func pointer name: token " +
                                  str(token) + " not expected; expected '*'")
        elif state == 1:
            if token == COp("*"):
                curCObj.ptrLevel += 1
            elif isinstance(token, CIdentifier):
                ...
                stateStruct.error("cpre3 parse func pointer name: token " +
                                  str(token) + " not expected; expected identifier")
    ...
    stateStruct.error(
        "cpre3 parse func pointer name: incomplete, missing ')' on level " + str(curCObj._bracketlevel))
    ...
    for token in input_iter:
        if isinstance(token, CIdentifier):
            if state == 0:
                ...
                stateStruct.error("cpre3 parse enum: unexpected identifier " +
                                  token.content + " after " + str(curCObj) + " in state " + str(state))
        elif token == COp("="):
            if state == 1:
                ...
                stateStruct.error(
                    "cpre3 parse enum: unexpected op '=' after " + str(curCObj) + " in state " + str(state))
        elif token == COp(","):
            if state in (1, 2):
                if state == 2:
            ...
            if not _isBracketLevelOk(parentCObj._bracketlevel, token.brackets):
                stateStruct.error(
                    "cpre3 parse enum: internal error: bracket level messed up with closing bracket: " + str(token.brackets))
        elif state == 2:
            if isinstance(token, COpeningBracket):
                valueStmnt._cpre3_parse_brackets(
                    stateStruct, token, input_iter)
            ...
            stateStruct.error("cpre3 parse enum: unexpected token " +
                              str(token) + " in state " + str(state))
    stateStruct.error(
        "cpre3 parse enum: incomplete, missing '}' on level " + str(parentCObj._bracketlevel))
...
def _cpre3_parse_skipbracketcontent(stateStruct, bracketlevel, input_iter):
    for token in input_iter:
        if isinstance(token, CClosingBracket):
            ...
            if not _isBracketLevelOk(bracketlevel, token.brackets):
                stateStruct.error(
                    "cpre3 parse skip brackets: internal error: bracket level messed up with closing bracket: " + str(token.brackets))
    stateStruct.error(
        "cpre3 parse: incomplete, missing closing bracket on level " + str(bracketlevel))
    ...
    for token in input_iter:
        if isinstance(token, CIdentifier):
            if token.content == "typedef":
                stateStruct.error(
                    "cpre3 parse func args: typedef not expected")
            elif token.content in stateStruct.Attribs:
                ...
                if curCObj.name is None:
                    ...
                    stateStruct.error(
                        "cpre3 parse func args: second identifier name " + token.content + " for " + str(curCObj))
        elif isinstance(token, COp):
            if token.content == ",":
                curCObj.finalize(stateStruct)
                curCObj = CFuncArgDecl(parent=parentCObj)
            ...
            if token.content == "(":
                if len(curCObj._type_tokens) == 1 and isinstance(curCObj._type_tokens[0], CFuncPointerDecl):
                    ...
                    curCObj._type_tokens[:] = [typeObj]
                    cpre3_parse_funcpointername(
                        stateStruct, typeObj, input_iter)
                    curCObj.name = typeObj.name
                else:
                    stateStruct.error(
                        "cpre3 parse func args: got unexpected '(' in " + str(curCObj))
                    _cpre3_parse_skipbracketcontent(
                        stateStruct, curCObj._bracketlevel, input_iter)
            elif token.content == "[":
                cpre3_parse_arrayargs(stateStruct, curCObj, input_iter)
            else:
                stateStruct.error(
                    "cpre3 parse func args: unexpected opening bracket '" + token.content + "'")
                _cpre3_parse_skipbracketcontent(
                    stateStruct, curCObj._bracketlevel, input_iter)
        elif isinstance(token, CClosingBracket):
            if token.brackets == parentCObj._bracketlevel:
            ...
            if not _isBracketLevelOk(parentCObj._bracketlevel, token.brackets):
                stateStruct.error(
                    "cpre3 parse func args: internal error: bracket level messed up with closing bracket: " + str(token.brackets))
            # no error. we already errored on the opening bracket. and the cpre2 parsing ensures the rest
        else:
            stateStruct.error(
                "cpre3 parse func args: unexpected token " + str(token))

    stateStruct.error(
        "cpre3 parse func args: incomplete, missing ')' on level " + str(parentCObj._bracketlevel))


def cpre3_parse_arrayargs(stateStruct, curCObj, input_iter):
    valueStmnt = CStatement()
    valueStmnt._bracketlevel = curCObj._bracketlevel
    valueStmnt._cpre3_parse_brackets(stateStruct, COpeningBracket(
        "[", brackets=curCObj._bracketlevel), input_iter)
    assert isinstance(valueStmnt._leftexpr, CArrayStatement)
    if isinstance(curCObj, (CVarDecl, CFuncArgDecl, CFuncPointerDecl)):
        arrayType = make_type_from_typetokens(
            stateStruct, curCObj, curCObj._type_tokens)
    ...
    for token in input_iter:
        if state == 0:
            if isinstance(token, CIdentifier):
                if token.content == "typedef":
                    stateStruct.error(
                        "cpre3 parse typedef: typedef not expected twice")
                elif token.content in stateStruct.Attribs:
                    curCObj.attribs += [token.content]
                elif token.content == "struct":
                    ...
                    if typeObj is not None and not typeObj._finalized and typeObj.name is None:
                        ...
                        if curCObj.name is None:
                            ...
                            stateStruct.error(
                                "cpre3 parse in typedef: got second identifier " + token.content + " after name " + curCObj.name)
                    else:
                        stateStruct.error(
                            "cpre3 parse in typedef: got unexpected identifier " + token.content)
            elif token == COp("*"):
                ...
                if token.content == "(":
                    ...
                    if isinstance(typeObj, CStruct):
                        ...
                        stateStruct.error(
                            "cpre3 parse in typedef: got unexpected '{' after type " + str(typeObj))
                        state = 11
                else:
                    stateStruct.error("cpre3 parse in typedef: got unexpected opening bracket '" +
                                      token.content + "' after type " + str(typeObj))
                    state = 11
            elif isinstance(token, CSemicolon):
                if typeObj is not None and not typeObj._finalized:
                    typeObj.finalize(
                        stateStruct, addToContent=typeObj.body is not None)
                curCObj.finalize(stateStruct)
                return
            else:
                stateStruct.error(
                    "cpre3 parse typedef: got unexpected token " + str(token))
            ...
            if isinstance(token, CClosingBracket):
                ...
                if not _isBracketLevelOk(curCObj._bracketlevel, token.brackets):
                    stateStruct.error(
                        "cpre3 parse typedef: internal error: bracket level messed up with closing bracket: " + str(token.brackets))
        else:
            stateStruct.error(
                "cpre3 parse typedef: internal error. unexpected state " + str(state))
...
def _getLastCBody(base):
    ...
    while True:
        if isinstance(base.body, CBody):
            ...
            break
        if not isinstance(last, _CControlStructure):
            break
...
class _CControlStructure(_CBaseWithOptBody):
    ...
    def asCCode(self, indent=""):
        ...
        if self.args:
            s += "(" + "; ".join(map(asCCode, self.args)) + ")"
        if self.body:
            s += "\n" + asCCode(self.body, indent)
        if hasattr(self, "whilePart"):
            s += "\n" + asCCode(self.whilePart, indent)
        if hasattr(self, "elsePart"):
        ...
        if isinstance(self.parent.body, CBody) and self.parent.body.contentlist:
            ...
            if isinstance(last, CDoStatement) and not last.whilePart:
                if self.body is not None:
                    stateStruct.error(
                        "'while' " + str(self) + " as part of 'do' " + str(last) + " has another body")
                last.whilePart = self
                addToContent = False
    ...
    def finalize(self, stateStruct, addToContent=False):
        ...
        while True:
            ...
            if last.elsePart is not None:
                ...
                if isinstance(lastIf.body, CBody):
                    break
        ...
        if lastIf is not None:
            ...
            stateStruct.error("'else' " + str(self) +
                              " without 'if', last was " + str(last))
        _CControlStructure.finalize(self, stateStruct, addToContent)
        ...
        if isinstance(token, CIdentifier):
            if isinstance(curCObj, CStatement):
                ...
                if curCObj.name is None:
                    ...
                    stateStruct.error("cpre3 parse statements in brackets: second identifier name " +
                                      token.content + ", first was " + curCObj.name + ", first might be an unknwon type")
            ...
            if isinstance(curCObj, CStatement):
                ...
                curCObj.body._cpre3_parse_brackets(
                    stateStruct, token, input_iter)
            elif not curCObj.isDerived():
                _make_statement(curCObj)
                curCObj._cpre3_parse_brackets(stateStruct, token, input_iter)
            else:
                stateStruct.error("cpre3 parse statements in brackets: " +
                                  str(token) + " not expected after " + str(curCObj))
                # fallback
                CStatement.overtake(curCObj)
                curCObj._cpre3_parse_brackets(stateStruct, token, input_iter)
        elif isinstance(token, CClosingBracket):
            if token.brackets == brackets:
                break
            stateStruct.error("cpre3 parse statements in brackets: unexpected closing bracket '" +
                              token.content + "' after " + str(curCObj) + " at bracket level " + str(brackets))
        elif token == sepToken:
            _finalizeCObj(curCObj)
            addToList.append(curCObj)
            curCObj = _CBaseWithOptBody(parent=parentCObj)
        # if the sepToken is not the semicolon, we don't expect it at all
        elif isinstance(token, CSemicolon):
            stateStruct.error(
                "cpre3 parse statements in brackets: ';' not expected, separator should be " + str(sepToken))
    ...
    for token in input_iter:
        if isinstance(token, COpeningBracket):
            ...
            if isinstance(curCObj, CStatement):
                ...
                curCObj._bracketlevel = list(token.brackets)
                if token.content == "(":
                    cpre3_parse_statements_in_brackets(stateStruct, curCObj, sepToken=CSemicolon(
                    ), addToList=curCObj.args, input_iter=input_iter)
                    curCObj._bracketlevel = list(parentCObj._bracketlevel)
                    lasttoken = cpre3_parse_single_next_statement(
                        stateStruct, curCObj, input_iter)
                    curCObj.finalize(stateStruct)
                    parentCObj.addToBody(curCObj)
                    return lasttoken
                elif token.content == "[":
                    stateStruct.error(
                        "cpre3 parse single after " + str(curCObj) + ": got unexpected '['")
                    _cpre3_parse_skipbracketcontent(
                        stateStruct, list(token.brackets), input_iter)
                    return
                elif token.content == "{":
                    if curCObj.body is not None:
                        stateStruct.error(
                            "cpre3 parse single after " + str(curCObj) + ": got multiple bodies")
                    cpre3_parse_body(stateStruct, curCObj, input_iter)
                    curCObj.finalize(stateStruct)
                    parentCObj.addToBody(curCObj)
                    return
                else:
                    stateStruct.error("cpre3 parse single after " + str(curCObj) +
                                      ": got unexpected/unknown opening bracket '" + token.content + "'")
                    _cpre3_parse_skipbracketcontent(
                        stateStruct, list(token.brackets), input_iter)
                    return
            else:
                stateStruct.error("cpre3 parse single: unexpected opening bracket '" +
                                  token.content + "' after " + str(curCObj))
        elif isinstance(token, CClosingBracket):
            if token.brackets == parentCObj._bracketlevel:
                stateStruct.error(
                    "cpre3 parse single: closed brackets without expected statement")
                return token
            stateStruct.error("cpre3 parse single: unexpected closing bracket '" + token.content +
                              "' after " + str(curCObj) + " at bracket level " + str(parentCObj._bracketlevel))
        elif isinstance(token, CSemicolon):
            ...
            if curCObj.name is None:
                ...
                stateStruct.error(
                    "cpre3 parse single after %s: got second identifier %s" % (curCObj, token))
            ...
            stateStruct.error(
                "cpre3 parse after %s: didn't expected identifier %s" % (curCObj, token))
        elif curCObj is None:
            curCObj = CStatement(parent=parentCObj)
            curCObj._cpre3_handle_token(stateStruct, token)
        else:
            stateStruct.error(
                "cpre3 parse single: got unexpected token %s after %s" % (token, curCObj))
        ...
        if isinstance(token, CIdentifier):
            if isinstance(curCObj, CStatement):
                ...
                if curCObj.name is None:
                    ...
                    stateStruct.error("cpre3 parse after " + str(curCObj) +
                                      ": got second identifier '" + token.content + "'")
                ...
                stateStruct.error("cpre3 parse after " + str(curCObj) +
                                  ": didn't expected identifier '" + token.content + "'")
                ...
                if curCObj.isDerived() or curCObj:
                    stateStruct.error("cpre3 parse: got '" +
                                      token.content + "' after " + str(curCObj))
                ...
                if curCObj.name is None:
                    ...
                    if DictName is not None:
                        typeObj = findCObjTypeInNamespace(
                            stateStruct, parentCObj, DictName, curCObj.name)
                        if typeObj is not None and typeObj.body is not None:  # if body is None, we still wait for another decl
                            curCObj = CVarDecl(parent=parentCObj)
                            curCObj._type_tokens += [typeObj]
                else:
                    stateStruct.error("cpre3 parse: second identifier name " + token.content +
                                      ", first was " + curCObj.name + ", first might be an unknwon type")
            ...
            if isinstance(curCObj, CStatement):
                ...
                if curCObj._finalized:  # might have been finalized internally. e.g. in case it was a goto-loop
                    curCObj = _CBaseWithOptBody(parent=parentCObj)
            # op(,) gets some extra handling. eg for CVarDecl
                ...
                # for example, because of op(,), we might have missed that above
                if isinstance(curCObj.body, CStatement):
                    curCObj.body._cpre3_handle_token(stateStruct, token)
                else:
                    stateStruct.error("cpre3 parse after " + str(curCObj) +
                                      ": didn't expected op '" + token.content + "'")
            else:
                if token.content == "*":
                    ...
                    stateStruct.error("cpre3 parse: op '" + token.content +
                                      "' not expected in " + str(parentCObj) + " after " + str(curCObj))
            ...
            if isinstance(curCObj, CVarDecl) and hasattr(curCObj, "bitsize"):
                ...
                stateStruct.error("cpre3 parse after " + str(curCObj) +
                                  ": didn't expected number '" + str(token.content) + "'")
            else:
                CStatement.overtake(curCObj)
                curCObj._cpre3_handle_token(stateStruct, token)
        elif isinstance(token, COpeningBracket):
            curCObj._bracketlevel = list(token.brackets)
            if not _isBracketLevelOk(parentCObj._bracketlevel, token.brackets):
                stateStruct.error("cpre3 parse body: internal error: bracket level messed up with opening bracket: " + str(
                    token.brackets) + " on level " + str(parentCObj._bracketlevel) + " in " + str(parentCObj))
            if isinstance(curCObj, CStatement):
                if token.content == "{":
                    cpre3_parse_body(stateStruct, curCObj, input_iter)
                    curCObj.finalize(stateStruct)
                ...
                if token.content == "(":
                    ...
                    curCObj._bracketlevel = list(
                        parentCObj._bracketlevel or [])
                    lasttoken = cpre3_parse_single_next_statement(
                        stateStruct, curCObj, input_iter)
                    curCObj.finalize(stateStruct)
                    if isinstance(lasttoken, CClosingBracket) and lasttoken.brackets == parentCObj._bracketlevel:
                    ...
                    stateStruct.error("cpre3 parse after " +
                                      str(curCObj) + ": got unexpected '['")
                    _cpre3_parse_skipbracketcontent(
                        stateStruct, list(token.brackets), input_iter)
                elif token.content == "{":
                    if curCObj.body is not None:
                        stateStruct.error(
                            "cpre3 parse after " + str(curCObj) + ": got multiple bodies")
                    cpre3_parse_body(stateStruct, curCObj, input_iter)
                    curCObj.finalize(stateStruct)
                    curCObj = _CBaseWithOptBody(parent=parentCObj)
                else:
                ...
                if curCObj.isDerived():
                    if isinstance(curCObj, CStruct):
                        ...
                        stateStruct.error(
                            "cpre3 parse: unexpected '{' after " + str(curCObj))
                        curCObj = _CBaseWithOptBody(parent=parentCObj)
                else:
                ...
                stateStruct.error(
                    "cpre3 parse: unexpected opening bracket '" + token.content + "'")
        elif isinstance(token, CClosingBracket):
            if token.content == "}":
                curCObj.finalize(stateStruct)
                curCObj = _CBaseWithOptBody(parent=parentCObj)
            else:
                stateStruct.error("cpre3 parse: unexpected closing bracket '" +
                                  token.content + "' after " + str(curCObj))
            if token.brackets == parentCObj._bracketlevel:
                return
            if not _isBracketLevelOk(parentCObj._bracketlevel, token.brackets):
                stateStruct.error("cpre3 parse body: internal error: bracket level messed up with closing bracket: " + str(
                    token.brackets) + " on level " + str(parentCObj._bracketlevel) + " in " + str(parentCObj))
        elif isinstance(token, CSemicolon):
            ...
            if isinstance(curCObj, CStatement):
                ...
                stateStruct.error(
                    "cpre3 parse after " + str(curCObj) + ": didn't expected " + str(token))
            elif not curCObj:
                CStatement.overtake(curCObj)
                curCObj._cpre3_handle_token(stateStruct, token)
            else:
                stateStruct.error("cpre3 parse: unexpected str " +
                                  str(token) + " after " + str(curCObj))
        else:
            stateStruct.error("cpre3 parse: unexpected token " + str(token))

    if curCObj and not curCObj._finalized:
        stateStruct.error("cpre3 parse: unfinished " +
                          str(curCObj) + " at end of " + str(parentCObj))

    if parentCObj._bracketlevel is not None:
        stateStruct.error("cpre3 parse: read until end without closing brackets " +
                          str(parentCObj._bracketlevel) + " in " + str(parentCObj))
...
def isPointerType(t, checkWrapValue=False, alsoFuncPtr=False, alsoArray=True):
    ...
    from inspect import isclass
    if alsoFuncPtr:
        if isinstance(t, CWrapFuncType):
            return True
        if isinstance(t, CFuncPointerDecl):
            return True
        if isclass(t) and issubclass(t, ctypes._CFuncPtr):
            return True
    if isclass(t):
        if issubclass(t, _ctypes._Pointer):
            return True
        if issubclass(t, ctypes.c_void_p):
            return True
    return False
...
def isValueType(t):
    if isinstance(t, (CBuiltinType, CStdIntType)):
        return True
    from inspect import isclass
    if isclass(t):
